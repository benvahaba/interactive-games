...
extends GameBoard {
	...
	public CatchTheBunny(int rowLength, int colLength) {
		...
		for(int row=0;row<board.length;row++)// fills the board with blankes before choosing random locations for computer and player
		...
		computerLocation=new GameMove(rand.nextInt(9),rand.nextInt(9));// choose a random place for the computer without any dependencies except the board bounds
		board[computerLocation.getRow()][computerLocation.getColumn()]=BoardSigns.COMPUTER;
		
		do//////// choose a random location for the player as long as its not right next to the computer or in his location and by the board bounds
		{
		...
		public boolean	updatePlayerMove(GameBoard.GameMove move)
		{
			...
			if(move.getRow()<0||move.getRow()>=board.length||move.getColumn()<0||move.getColumn()>=board[0].length// the move is illegal if its out of bounds or 2 steps away from the last location
			||
